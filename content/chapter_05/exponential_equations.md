...
Seek a common base before using the one-to-one property.
...
exponent rules and equation solving.
...
## L0 Foundation
Repair exponent rules and equation solving in isolation.
...
1. A minimally guided symbolic exponential equations prompt with the method unstated.
2. A exponential equations representation bridge that changes notation or context while preserving the same first decision.
3. A cumulative exponential equations prompt with one prerequisite and the required final-form check.
...
Present an exact solution set, with checks required by the original operations.
