...
- learner-facing mathematical notation from `math_display.py`.
...
## 23. `src/math153_tutor/math_display.py`

### A. Purpose

Keeps validator syntax and learner-facing notation separate. The validator can continue using unambiguous
strings such as `5*sqrt(2)`, while the UI displays $5\sqrt{2}$.

### B/C. Connections and walkthrough

`app.py` imports the module for prompts, worked steps, feedback, and answer choices. `answer_to_latex`
handles equations, sets, intervals, coordinates, complex values, and symbolic expressions through SymPy.
`normalize_math_fragment` cleans template syntax inside existing LaTeX fragments. `readable_prompt` changes
only delimited mathematics and preserves professor/model wording. `display_choices` pairs each raw answer
with a letter and LaTeX presentation; the raw value is still what validation and persistence receive.

### D–F. Example/problems/changes

The raw choice `4*x+(-9)` is rendered in a labeled math card as $4x-9$; selecting its letter still submits
the exact raw expression to the existing validator. This fixes the presentation bug without weakening
answer-form checks.

## 24. `src/math153_tutor/build_validation.py`

This module is the fail-closed build gate. `BuildValidationReport` records discovered/read sources, empty
content, parsed families, generated examples, validated answers, empty outputs, unresolved inventory
sources, and exact errors. `validate_build` reads every canonical family body, parses it, checks source
references, generates five examples from every runtime family, validates every expected answer, requires
worked steps, and scans derived outputs for zero-byte files. Missing external binaries remain unresolved
rather than being mislabeled complete.

## 25. `scripts/validate_build.py`

This small command-line adapter runs the build gate at the repository root, prints the complete JSON
report, and exits with status 1 when any failure is present. A successful run is therefore suitable for
local development or continuous integration.
...
## `src/math153_tutor/corpus.py`

**Purpose.** This is the immutable-source ingestion boundary. It never creates, moves, or edits `source/`. It recursively discovers source files, reads supported text bodies, extracts conservative evidence (headings, question blocks, and LaTeX formulas), and returns typed `ExtractedSource` records. Binary images and PDFs are preserved as manifest entries with partial status rather than passed off as extracted text.

**Connections and flow.** `app.py` calls `ingest_source_corpus` for the Source Inspector; `build_validation.py` uses it for fail-closed source checks; `scripts/rebuild_math153_corpus.py` writes its manifest records. `extract_source_file` hashes the original, classifies its filename through `source_manifest.py`, reads text only for supported suffixes, and records exactly how many question/formula records were found.

**Example.** Adding `source/quizzes/Quiz_5.md` and running the rebuild reads its body, retains the raw Markdown for inspection, extracts heading topics and question/formula evidence, hashes the file, and writes a traceable manifest row. A missing `source/` produces an empty result without manufacturing replacement material.

## `scripts/rebuild_math153_corpus.py`

**Purpose.** This is the reproducible source-first rebuild command. It ingests the immutable corpus, writes the body-derived CSV manifest, prints extraction counts, and exits nonzero if the corpus is absent, empty, or still contains files requiring extraction.

**Connections and flow.** The script uses repository-root paths from `paths.py`, extraction from `corpus.py`, and CSV serialization from `source_manifest.py`. It is intentionally stricter than the legacy filename-inventory builder.
