...
When the authoritative immutable `source/` corpus is present, rebuild the body-derived manifest with:

```bash
PYTHONPATH=src python scripts/rebuild_math153_corpus.py
```

This command fails rather than manufacturing a `source/` directory when the corpus is absent, and it
fails when a preserved binary still needs text/OCR extraction. Course Review includes a Source Inspector
that exposes the same evidence and never represents inventory-only filenames as ingested content.
