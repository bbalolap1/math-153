...
def test_present_markdown_body_is_read_and_measured(tmp_path: Path) -> None:
    registry = tmp_path / "registry.txt"
    registry.write_text("source/Quiz_7.md\n", encoding="utf-8")
    source_root = tmp_path / "root"
    source = source_root / "source/Quiz_7.md"
    source.parent.mkdir(parents=True)
    source.write_text("# Quiz 7\n\nQuestion: Find $x$ when $2x=8$?\n", encoding="utf-8")

    entry = build_inventory_manifest([registry], source_root)[0]

    assert entry.extraction_status == "complete"
    assert entry.content_length > 0
    assert entry.questions_present
    assert entry.formulas_present
