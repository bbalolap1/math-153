...
from math153_tutor.math_display import answer_to_latex
...
def _app(tmp_path: Path, monkeypatch) -> AppTest:
    ...
    monkeypatch.setenv("MATH153_DATA_DIR", str(tmp_path / "learner_data"))
...
def test_course_review_opens_readable_family_content(tmp_path: Path, monkeypatch) -> None:
    ...
    assert any("authoritative source/ directory is absent" in item.value for item in app.error)
...
def test_paper_mode_domain_family_shows_full_solution_and_new_variation(
    tmp_path: Path, monkeypatch
) -> None:
    app = _app(tmp_path, monkeypatch)
    _navigation(app).set_value("Learning Tools").run()
    next(button for button in app.button if button.label == "Open Paper Answer Mode").click().run()
    first = app.session_state["practice_tool_problem"]
    assert first.family_id == "CH34-DOMAIN-RADICAL-RATIONAL"
    assert any(r"\frac{\sqrt{" in item.value for item in app.markdown)

    next(button for button in app.button if button.label == "Show solution").click().run()
    assert any("Requested solution" in item.value for item in app.markdown)
    assert any("\\boxed" in item.value for item in app.latex)

    next(button for button in app.button if button.label == "New variation").click().run()
    second = app.session_state["practice_tool_problem"]
    assert second.family_id == first.family_id
    assert second.parameter_seed != first.parameter_seed
    assert second.prompt != first.prompt


def test_paper_answer_choice_labels_are_in_dom_order(tmp_path: Path, monkeypatch) -> None:
    app = _app(tmp_path, monkeypatch)
    _navigation(app).set_value("Learning Tools").run()
    next(button for button in app.button if button.label == "Open Paper Answer Mode").click().run()

    labels = [item.value for item in app.markdown if item.value in {"**A.**", "**B.**", "**C.**", "**D.**"}]
...
def test_review_routes_real_mistake_through_transfer_repair(tmp_path: Path, monkeypatch) -> None:
    ...
    _navigation(app).set_value("Progress").run()
    repair_metric = next(metric for metric in app.metric if metric.label == "Repair checks")
    assert repair_metric.value == "1"
...
def test_graph_selection_is_validated_and_persisted(tmp_path: Path, monkeypatch) -> None:
    app = _app(tmp_path, monkeypatch)
    _navigation(app).set_value("Quiz / Test / Exam").run()
    next(item for item in app.multiselect if item.label == "Select the plotted intercept coordinates").set_value(
        ["(-2, 0)", "(2, 0)"]
    ).run()
    next(button for button in app.button if button.label == "Check graph selection").click().run()

    assert any("Correct" in message.value for message in app.success)
    assert (tmp_path / "learner_data/practice_attempts.sqlite3").is_file()


def test_professor_mode_preserves_unsent_assessment_selection(tmp_path: Path, monkeypatch) -> None:
    app = _app(tmp_path, monkeypatch)
    _navigation(app).set_value("Quiz / Test / Exam").run()
    next(button for button in app.button if button.label == "Launch Assessment").click().run()
    problem = app.session_state["assessment_problems"][0]
    answer = next(radio for radio in app.radio if radio.label == "Select one answer")
    answer.set_value(problem.expected_answer).run()
    next(button for button in app.button if button.label == "Ask Professor Mode").click().run()
    next(button for button in app.button if button.label == "Return to active assessment").click().run()

    restored = next(radio for radio in app.radio if radio.label == "Select one answer")
    assert restored.value == problem.expected_answer
    assert app.session_state["assessment_index"] == 0
