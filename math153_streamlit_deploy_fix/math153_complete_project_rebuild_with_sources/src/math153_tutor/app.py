...
def _style() -> None:
    st.markdown(
        ...
        .mn-review-card {border:1px solid rgba(128,128,128,.25); border-radius:.7rem; padding:1rem; margin-bottom:1rem;}
        </style>
        """,
...
def _render_prompt(prompt: str) -> None:
    """Render professor wording as prose and mathematical regions as proper display math."""
...
def _course_review() -> None:
    ...
    section=st.selectbox("Section",sections,format_func=lambda s:f"{s.id} — {s.title}")
    ...
    if section.formulas:
        for formula in section.formulas:
            ...
            if formula.conditions:
                st.caption("Conditions: "+"; ".join(formula.conditions))
    else:
        st.caption("No standalone formula is required for this section.")

    st.markdown("### Procedures")
    for procedure in section.procedures:
        with st.expander(procedure.name):
            st.write(procedure.objective)
            for i,x in enumerate(procedure.steps,1):
                st.markdown(f"{i}. {x}")
    ...
    for family in section.problem_families:
        with st.expander(family.title):
            ...
            for item in family.recognition_features:
                st.markdown(f"- {item}")
            st.markdown("**Common mistakes**")
            for item in family.common_mistakes:
                st.markdown(f"- {item}")
...
def _render_answer_widget(problem: PracticeProblem, key: str) -> str:
    ...
    if problem.answer_type in {"multiple_choice","multi_select"} or choices:
        ...
        # Keep answer choices simple and readable. The selected raw value is returned for grading.
...
def _show_solution(problem: PracticeProblem) -> None:
    ...
    if solution.decide:
        st.markdown(f"**Method:** {solution.decide}")
    if solution.original_rule_or_formula:
        st.markdown("**Rule / formula**")
        _render_solution_expression(solution.original_rule_or_formula)
    ...
    if solution.verify:
        st.markdown(f"**Check:** {solution.verify}")
...
def _professor_mode() -> None:
    ...
    if "prof_variant" not in st.session_state:
        st.session_state.prof_variant=4
    if "prof_seed" not in st.session_state:
        st.session_state.prof_seed=random.randint(1,10_000_000)
    ...
    if st.button("Check answer",key=f"prof-submit-{problem.problem_id}",type="primary"):
        ...
        PracticeAttemptRepository(PRACTICE_DB).add(
            PracticeAttempt(problem_id=problem.problem_id,family_id=problem.family_id,answer=answer,correct=result.correct,answer_type=problem.answer_type,shown_work="")
        )
...
def _assessment_review(kind: str, results) -> None:
    ...
    st.caption("Open each question to compare your answer with the correct answer and review the solution.")
...
def _assessment_page(kind: str, default_count: int) -> None:
    ...
    if source_mode!="Custom coverage":
        st.caption(f"Using the source-supported pattern for **{source_mode}**.")
    ...
    if index>=len(questions):
        ...
        if not st.session_state.get(f"{kind}-saved",False):
            ...
            st.session_state[f"{kind}-saved"]=True
        _assessment_review(kind,results)
        return
    ...
    if st.button("Submit and continue",key=f"{kind}-submit-{problem.problem_id}",type="primary"):
        if not answer.strip():
            st.warning("Select or enter an answer first.")
            return
        result=grade_question(problem,answer,shown_work="")
...
def _progress() -> None:
    ...
    if records:
        rows=[]
        for r in records:
            rows.append({"type":r.assessment_type,"date":r.completed_at.isoformat(timespec="minutes"),"score":f"{r.correct_count}/{len(r.question_results)}","accuracy":round(r.accuracy,3)})
